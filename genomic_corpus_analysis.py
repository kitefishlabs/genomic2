...
#	create corpus
anchorpath = os.path.expanduser('~/dev/git/genomic2')
...
for a, sndA in enumerate(allfiles):
	...
	for sndB in sorted(list(set(allfiles).difference(set([allfiles[a]])))):
		...
		genex = GenomicExplorer(anchorpath, [os.path.basename(sndA), os.path.basename(sndB)], subdir='combos',size=50, margin=10, report_interval=5, mut_prob=0.02, stop_slope=0.001)
